...
public class DepthImageViewer : MonoBehaviour 
{
	...
	void Start () 
	{
        ...
        Rect cameraRect = player.GetComponent<Camera>().pixelRect;
		...
		foregroundRect = new Rect(foregroundOfs.x, cameraRect.height - foregroundOfs.y + 160, rectWidth, -rectHeight);
			...
			for(int i = 0; i < numColliders; i++)
			{
				if(manager.IsJointTracked(userId, i))
				{
					...
					if(posJoint != Vector3.zero)
					{
						...
						float screenY = player.GetComponent<Camera>().pixelHeight - (foregroundOfs.y + scaledY);
						float zDistance = posJoint.z - player.GetComponent<Camera>().transform.position.z;
						
						Vector3 posScreen = new Vector3(screenX, screenY, zDistance);
						Vector3 posCollider = player.GetComponent<Camera>().ScreenToWorldPoint(posScreen);
